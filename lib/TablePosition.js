...
class TablePosition extends Immutable.Record(DEFAULTS) {
    ...
    static create(value, startBlock, opts) {
        ...
        const headerCell = value.document.getClosest(startBlock.key, (p) => (p.type === opts.typeHeaderCell));
    ...
    static isInCell(value, startBlock, opts) {
        return (
            ...
            startBlock.type === opts.typeHeaderCell ||
            value.document.getClosest(startBlock.key, (p) => (p.type === opts.typeHeaderCell))
        ) ? true : false;
    }
}
